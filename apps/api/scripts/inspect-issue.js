...
if (!ATLASSIAN_EMAIL || !ATLASSIAN_API_TOKEN || !JIRA_DOMAIN) {
  console.error(
    'Error: Please set ATLASSIAN_EMAIL, ATLASSIAN_API_TOKEN, and JIRA_DOMAIN environment variables.',
  );
...
function fetchAllowedValues(fieldId) {
  ...
  const metaReq = https.request(editMetaOptions, (res) => {
    ...
    res.on('end', () => {
      ...
      if (meta.fields[fieldId]?.allowedValues) {
        ...
        console.log(
          'Field might not be a Select List or user does not have permission to edit it.',
        );
      }
    });
  });
